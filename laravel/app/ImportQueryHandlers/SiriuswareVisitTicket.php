...
class SiriuswareVisitTicket extends ImportQueryHandler {
	...
	function process() {
		...
		$cols = ['v.id', 'source_id', 'v.venue_id', 'w.id as workstation_id', 'f.id as facility_id', 'p.id as product_id', 'ticket_code'
		...
		straight_join product as p on box_office_product_code = p.code and v.venue_id = p.venue_id and p.system_id = $system_id
